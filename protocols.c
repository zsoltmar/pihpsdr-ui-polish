...
//
// protocols_quiet_radios()
//
// Best-effort recovery from a previous crash of piHPSDR.
//
// If the previous instance died before sending its protocol-stop command,
// the radio firmware (HPSDR/Hermes/Hermes-Lite/Red Pitaya/...) will keep
// streaming sample UDP packets to the now-dead client endpoint. Those
// packets often jam the next discovery exchange (the discovery socket
// receives stream junk instead of a discovery response), which is why a
// crashed piHPSDR usually needs a host reboot before it works again.
//
// Sending a protocol-1 "Stop" command (EF FE 04 00, padded to 64 bytes)
// as a broadcast on UDP/1024 makes any HPSDR radio on the LAN stop
// streaming. Protocol-2 radios ignore it (their stop is on a different
// port), but in practice quieting the much more common P1 stream is
// enough to let discovery proceed cleanly.
//
// This is best-effort: we silently ignore every error and never block for
// more than ~250 ms.
//
void protocols_quiet_radios() {
  ...
  metis_stop[0] = 0xEF;
  metis_stop[1] = 0xFE;
  metis_stop[2] = 0x04;  // start/stop command
  metis_stop[3] = 0x00;  // 0 = stop EP6 + EP4
  ...
  for (int i = 0; i < 3; i++) {
    ...
    usleep(50000);  // 50 ms between bursts; 3 bursts = ~150 ms total
  }

  // Give the radio a moment to actually stop spewing samples.
