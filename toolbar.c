...
GtkWidget *toolbar_init(int my_width, int my_height, GtkWidget* parent) {
    ...
    // 8 buttons share the toolbar width, separated by 7 two-pixel gutters
    // (see column-spacing below). Subtract the gutter total first so the
    // per-button min-width doesn't push the grid beyond the screen edge.
    int button_width=(width - 7 * 2) / 8;
    ...
    gtk_widget_set_name(toolbar, "pihpsdr_toolbar");
    // Leave 2 px below the button row so it doesn't kiss the window edge.
    gtk_widget_set_size_request (toolbar, width, height - 2);
    gtk_grid_set_column_homogeneous(GTK_GRID(toolbar),TRUE);
    // 2 px gutters between neighbouring buttons.
    gtk_grid_set_column_spacing(GTK_GRID(toolbar), 2);
