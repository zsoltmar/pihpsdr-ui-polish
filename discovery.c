...
//
  ...
  //
  // Best-effort recovery from a previous crash: broadcast a
  // protocol-1 "stop" so any radio still streaming to a dead
  // endpoint quiets down before we try to discover it.
  //
  if (enable_protocol_1 || enable_protocol_2) {
    status_text("Quieting any stuck radios ...");
  ...
  status_text("Discovery");
    ...
    gtk_widget_set_name(discovery_dialog, "pihpsdr_discovery_dialog");
    pihpsdr_dialog_dark_background(discovery_dialog);
...
fprintf(stderr,"%p Protocol=%d name=%s\n",d,d->protocol,d->name);
        ...
        gtk_widget_set_name(label, "pihpsdr_discovery_label");
        gtk_widget_override_font(label, pango_font_description_from_string(SLIDERS_FONT));
        gtk_widget_set_halign (label, GTK_ALIGN_START);
        gtk_widget_show(label);
        gtk_grid_attach(GTK_GRID(grid),label,0,row,3,1);

        GtkWidget *start_button=gtk_button_new_with_label("Start");
        gtk_widget_set_name(start_button, "pihpsdr_discovery_action");
	  ...
	  //
        ...
        if (d->protocol == STEMLAB_PROTOCOL) {
          if (d->software_version == 0) {
            ...
            gtk_widget_override_font(apps_combobox[row], pango_font_description_from_string(SLIDERS_FONT));
