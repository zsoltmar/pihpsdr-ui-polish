...
static GtkWidget *mic_source_combo=NULL;
...
static void local_microphone_cb(GtkWidget *widget, gpointer data) {
  if(gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget))) {
    ...
    if(audio_open_input()==0) {
      ...
      if(mic_source_combo!=NULL) gtk_widget_hide(mic_source_combo);
      if(micboost_b!=NULL) gtk_widget_hide(micboost_b);
    } else {
      transmitter->local_microphone=0;
      gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (widget), FALSE);
      if(mic_source_combo!=NULL) gtk_widget_show(mic_source_combo);
...
static void mic_source_combo_changed(GtkComboBox *combo, gpointer data) {
  (void)data;
...
void tx_menu(GtkWidget *parent) {
  ...
  if(device_has_microphone_input) {
    ...
    GtkWidget *mic_src_label=gtk_label_new(NULL);
    gtk_label_set_markup(GTK_LABEL(mic_src_label), "<b>Radio input</b>");
#ifdef GTK316
    gtk_label_set_xalign(GTK_LABEL(mic_src_label),0);
#endif
    gtk_grid_attach(GTK_GRID(grid),mic_src_label,col,row,1,1);
    col++;

    mic_source_combo=gtk_combo_box_text_new();
    gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(mic_source_combo), NULL, "Mic in");
    gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(mic_source_combo), NULL, "Line in");
    gtk_combo_box_set_active(GTK_COMBO_BOX(mic_source_combo), mic_linein ? 1 : 0);
    gtk_widget_set_hexpand(mic_source_combo, TRUE);
    gtk_widget_set_halign(mic_source_combo, GTK_ALIGN_FILL);
    gtk_widget_show(mic_source_combo);
    gtk_grid_attach(GTK_GRID(grid),mic_source_combo,col,row,1,1);
    g_signal_connect(mic_source_combo, "changed", G_CALLBACK(mic_source_combo_changed), NULL);
