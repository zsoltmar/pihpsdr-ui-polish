...
#define VFO_HEIGHT (68)
#define VFO_WIDTH (display_width-METER_WIDTH-MENU_WIDTH)
#define METER_HEIGHT (68)
#define METER_WIDTH (200)
#define PANADAPTER_HEIGHT (105)
/* Must fit natural height of #pihpsdr_fill_scale rows (trough + handle margins). */
#define ZOOMPAN_HEIGHT (44)
#define SLIDERS_HEIGHT (88)
...
static void create_visual() {
  ...
  //
  // Install the page host: wraps `fixed` in a GtkStack so menus can
  // swap in as full pages instead of opening as separate dialogs.
  //
  pihpsdr_menu_page_host_install(top_window, fixed);
...
//g_print("radio: meter_init\n");
  ...
  gtk_widget_set_name(minimize_b, "pihpsdr_chrome_btn");
  ...
  gtk_widget_set_name(menu_b, "pihpsdr_chrome_btn");
...
*/
    ...
    */
    ...
    value=getProperty("pihpsdr_menu_animations");
    if(value!=NULL) pihpsdr_menu_animations=atoi(value);
...
g_print("radioSaveState: %s\n",property_path);
  ...
  if(!radio_is_remote) {
    ...
    */
    ...
    sprintf(value,"%d",pihpsdr_menu_animations);
    setProperty("pihpsdr_menu_animations",value);
#ifdef CLIENT_SERVER
  }
#endif
