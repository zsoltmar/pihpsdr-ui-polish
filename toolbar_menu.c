...
void toolbar_menu(GtkWidget *parent) {
  ...
  gtk_widget_set_name(notebook, "pihpsdr_menu_notebook");

next_function_set:

  grid=gtk_grid_new();
  gtk_widget_set_name(grid, "pihpsdr_toolbar_preview");
  gtk_grid_set_column_homogeneous(GTK_GRID(grid),TRUE);
  gtk_grid_set_row_homogeneous(GTK_GRID(grid),FALSE);
  // 2 px gutter mirrors the live dashboard toolbar so this page reads as a
  // real preview of what the bottom button row will look like.
