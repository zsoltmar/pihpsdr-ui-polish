...
GtkWidget *zoompan_init(int my_width, int my_height) {
  ...
  zoompan=gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 8);
  gtk_widget_set_name(zoompan, "pihpsdr_zoompan");
  gtk_widget_set_size_request(zoompan, width, height);
  gtk_box_set_homogeneous(GTK_BOX(zoompan), FALSE);
  ...
  zoom_row=pihpsdr_labeled_scale_row("Zoom", zoom_scale, NULL, TRUE);
  gtk_box_pack_start(GTK_BOX(zoompan), zoom_row, TRUE, TRUE, 0);
  ...
  pan_row=pihpsdr_labeled_scale_row("Pan", pan_scale, NULL, TRUE);
  gtk_box_pack_start(GTK_BOX(zoompan), pan_row, TRUE, TRUE, 0);
