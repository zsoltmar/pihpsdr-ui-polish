...
# Where `make install` copies the built binary (needs write permission, usually sudo).
# Usage: make && sudo make install
# Override: make install INSTALL_BINDIR=$$HOME/.local/bin
...
.PHONY:	install
install: $(PROGRAM)
	install -m 755 $(PROGRAM) $(INSTALL_BINDIR)/$(PROGRAM)

# Local desktop icon: runs the binary in THIS source tree (no sudo).
# Uses XDG desktop folder when xdg-user-dir exists.
XDG_DESKTOP := $(shell xdg-user-dir DESKTOP 2>/dev/null)
ifeq ($(strip $(XDG_DESKTOP)),)
...
install-desktop: $(PROGRAM)
	@set -e; d="$(DESKTOP_DIR)"; mkdir -p "$$d"; \
	exec="$(CURDIR)/$(PROGRAM)"; \
	sed -e "s|^Exec=.*|Exec=$${exec}|" -e "s|^TryExec=.*|TryExec=$${exec}|" pihpsdr.desktop > "$$d/pihpsdr.desktop.new"; \
	mv -f "$$d/pihpsdr.desktop.new" "$$d/pihpsdr.desktop"; \
	chmod 644 "$$d/pihpsdr.desktop"; \
	printf "install-desktop: %s/pihpsdr.desktop -> %s\n" "$$d" "$$exec"

# Full build + refresh desktop launcher (use this when you want the desktop icon to match the tree binary).
.PHONY:	compile
compile:
	@$(MAKE) all
