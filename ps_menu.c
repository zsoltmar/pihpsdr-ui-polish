...
//       
//
static void ps_ant_combo_changed(GtkComboBox *combo, gpointer data) {
  (void)data;
  static const int vals[] = { 0, 6, 7 };
  int i = gtk_combo_box_get_active(GTK_COMBO_BOX(combo));
  if (i < 0 || i >= (int)(sizeof(vals) / sizeof(vals[0]))) {
    return;
  }
  int val = vals[i];
  switch (val) {
...
void ps_menu(GtkWidget *parent) {
  ...
  //
  GtkWidget *ps_ant_label=gtk_label_new(NULL);
  gtk_label_set_markup(GTK_LABEL(ps_ant_label), "<b>PS feedback antenna</b>");
  ...
  {
    int ant = receiver[PS_RX_FEEDBACK]->alex_antenna;
    int sel = 0;
    if (ant == 6) {
      sel = 1;
    } else if (ant == 7) {
    ...
    gtk_combo_box_set_active(GTK_COMBO_BOX(ps_ant_combo), sel);
  }
  gtk_widget_set_hexpand(ps_ant_combo, TRUE);
  gtk_widget_set_halign(ps_ant_combo, GTK_ALIGN_FILL);
  gtk_widget_show(ps_ant_combo);
  gtk_grid_attach(GTK_GRID(grid), ps_ant_combo, col, row, 1, 1);
  g_signal_connect(ps_ant_combo, "changed", G_CALLBACK(ps_ant_combo_changed), NULL);
  col++;
