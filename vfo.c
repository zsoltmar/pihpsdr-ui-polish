            ...
            gpointer           data)
{
  ...
  /* Panel background (matches app chrome) */
  cairo_t *cr;
...
void vfo_update() {
    ...
    if(vfo_surface) {
        ...
        cairo_t *cr;
        cr = cairo_create (vfo_surface);
        pihpsdr_cairo_text_rendering(cr);
        cairo_set_source_rgb (cr, 0.07, 0.08, 0.11);
        cairo_paint (cr);
        ...
        cairo_set_source_rgb(cr, 0.62, 0.68, 0.78);
        cairo_move_to(cr, 5, 15);
        cairo_show_text(cr, temp_text);

        cairo_select_font_face(cr, DISPLAY_FONT_MONO,
            CAIRO_FONT_SLANT_NORMAL,
            CAIRO_FONT_WEIGHT_BOLD);
	...
	if(vfo[1].entering_frequency) {
        ...
        if(txvfo == 0 && (isTransmitting() || oob)) {
            ...
            if(vfo[0].entering_frequency) {
              cairo_set_source_rgb(cr, 0.98, 0.82, 0.38);
            } else if(id==0) {
              cairo_set_source_rgb(cr, 0.45, 0.88, 0.78);
            } else {
              cairo_set_source_rgb(cr, 0.28, 0.38, 0.42);
            }
        }
        cairo_move_to(cr, 5, 38);  
        cairo_set_font_size(cr, id==0 ? DISPLAY_FONT_SIZE5 : DISPLAY_FONT_SIZE4);
        cairo_show_text(cr, temp_text);
        ...
        cairo_set_font_size(cr, id==1 ? DISPLAY_FONT_SIZE5 : DISPLAY_FONT_SIZE4);
        cairo_show_text(cr, temp_text);
