...
/*
 * Hub page handle (the top-level "Menu" page in the page host).
 * When non-NULL the hub is currently on the stack.
 */
static GtkWidget *hub_page=NULL;
...
/*
 * Dismiss every menu page and return to the radio view.
 * Preserved the historical name so existing sub-menu callers
 * (start_band(), start_mode(), ...) still compile.
 */
static void cleanup() {
  sub_menu=NULL;
  hub_page=NULL;
...
static void hub_destroyed_cb(GtkWidget *w, gpointer data) {
  (void)w;
  (void)data;
  hub_page=NULL;
}
...
/*
 * Hub-button callbacks: push the sub-menu as a page on top of the hub.
 * Navigation Back pops to the hub; Close pops everything.
 */
...
/*
 * `start_*()` helpers are entry points used by external triggers
 * (toolbar buttons, GPIO encoders, ...) that need to slam straight
 * into a sub-menu without the user having gone through the hub.  They
 * tear down whatever menu is currently up and open the requested one
 * standalone.
 *
 * The hub-button callbacks below DO NOT call these helpers — they
 * invoke the menu functions directly so the hub stays on the page
 * stack and the sub-menu shows a working "Back" button.  Calling
 * start_*() from a hub button would close the hub before the
 * sub-menu's adapt-to-page idle fires, leaving the new page wrongly
 * classified as the hub (no Back button).
 */
...
void new_menu()
{
  ...
  //
  // Toggle behaviour: if the hub is already showing, clicking Menu
  // tears down all menu pages and returns to the radio view.
  //
  if(hub_page!=NULL) {
    cleanup();
    return;
  ...
  /*
   * First row mirrors the legacy hub but is now redundant with the
   * page header's Close button; we keep the Restart and Exit shortcuts
   * since they are destructive / discoverable actions.
   */
  i = 0;
...
//
  ...
  hub_page = pihpsdr_menu_page_push_widget("Menu", grid);
  if (hub_page != NULL) {
    g_signal_connect(hub_page, "destroy", G_CALLBACK(hub_destroyed_cb), NULL);
  }
}

void
pihpsdr_dialog_fit_parent(GtkWidget *dialog, GtkWidget *parent)
{
  gint pw = 800;
  gint ph = 480;
...
void
pihpsdr_dialog_pack_scrolled_body(GtkWidget *dialog, GtkWidget *child)
{
  GtkWidget *content;
  GtkWidget *sw;
  ...
  gtk_widget_set_name(sw, "pihpsdr_modal_scroll");
  ...
  gtk_scrolled_window_set_max_content_height(GTK_SCROLLED_WINDOW(sw), 380);
#endif
...
void
pihpsdr_modal_grid_defaults(GtkWidget *grid)
{
